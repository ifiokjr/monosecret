...
pub struct ProviderValue {
	...
	/// Non-secret identity and generation of these provider bytes (0.4.0+).
	pub revision: Option<monosecret_ipc::Revision>,
}
...
///
/// Provider values are arbitrary bytes starting with Monosecret 0.4.0. A
...
pub trait Provider: Send + Sync {
	...
	/// provider does not report through
	/// [`supports_coord`](Provider::supports_coord), so a store whose secrets
	/// have no sub-components gets the correct behavior from the empty default
	/// without writing any validation.
	fn supported_coords(&self) -> &'static [&'static str] {
		&[]
	}

	/// Returns whether this provider understands an optional native-address
	/// coordinate. Available since Monosecret 0.4.0.
	///
	...
	/// outside [`supports_coord`](Provider::supports_coord) are rejected,
	...
	/// Available since Monosecret 0.4.0.
	///
	...
	///
	/// Defaults to [`configured_entry_coordinates`](Provider::configured_entry_coordinates).
	/// Providers may read storage to resolve aliases here, so this is suitable
	/// for destructive preflight but must not be used during cache planning.
	fn entry_coordinates<'a>(&self, addr: Address<'a>) -> Result<Cow<'a, NativeAddress>> {
		self.configured_entry_coordinates(addr)
	}

	/// Resolves [`entry_coordinates`](Provider::entry_coordinates) for several
	/// addresses at once. Available since Monosecret 0.4.0.
	///
	...
	///
	/// Available since Monosecret 0.4.0. Existing providers inherit a
	...
	///
	/// Available since Monosecret 0.4.0. Write-only stores override this
	...
	///
	/// Available since Monosecret 0.4.0. Readable providers inherit the
	...
	fn configure_dependency_secrets(&self, _dependencies: &[(String, SecretBytes)]) -> Result<()> {
		Ok(())
	}
	...
	/// considered shared.
	///
	/// This may read provider storage through [`entry_coordinates`](Provider::entry_coordinates) and
	...
	/// [`physical_store_path`](Provider::physical_store_path) as known from
	/// configuration alone. Available since Monosecret 0.4.0.
	///
	/// Like [`configured_entry_coordinates`](Provider::configured_entry_coordinates),
	/// this must not contact the store; planning compares containers before
	/// consulting the cache. Providers that only learn their path from the
	/// store report what they already know, or `None`.
	...
	/// names that exist.
	///
	/// Available since Monosecret 0.4.0. Readable providers reuse their
