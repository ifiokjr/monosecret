...
use crate::{Result, Secret, MonosecretError};
use monosecret_ipc::deadline_unix_ms_after;
use monosecret_ipc::error::{ErrorKind as RpcErrorKind, RpcError};
use monosecret_ipc::lifecycle::{CredentialResponder, Environment, LaunchOptions, ProviderSession};
...
use monosecret_ipc::protocol::{Limits, Product};
...
fn ipc_error(error: monosecret_ipc::Error) -> MonosecretError {
    match error {
        monosecret_ipc::Error::Remote(error) => MonosecretError::ProviderProtocol {
            kind: error.data.kind,
            interaction: error.data.interaction,
        },
        error => match error.rpc_kind() {
            Some(kind) => MonosecretError::ProviderProtocol {
                kind,
                interaction: None,
            },
            None => MonosecretError::ProviderOperationFailed(error.stable_message().to_string()),
        },
    }
}
