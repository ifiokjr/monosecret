	...
	'session: loop {
		...
		let frame = {
			...
			loop {
				tokio::select! {
					biased;
					() = disconnected.cancelled() => break 'session,
					() = async {
...
where
	R: AsyncRead + Unpin,
{
	let params: InitializeParams<Value> = if let Ok(params) =
...
async fn commit_application_before(
	...
	let payload = if let Some(payload) = encoded.filter(|_| application_response) {
		payload
	} else {
	...
	if let Ok(Ok(Ok(()))) = tokio::time::timeout_at(deadline, committed_rx).await {
