	...
	pub(crate) async fn read_frame(&mut self, limit: usize) -> Result<Option<Zeroizing<Vec<u8>>>> {
		...
		loop {
			let (consumed, mut frames) = {
				...
				let chunk = available.get(..consumed).ok_or(Error::Protocol(
					"reader buffer ended before the frame boundary",
				))?;
	...
	async fn async_reader_reuses_one_buffered_chunk_across_frames() {
		...
		impl AsyncRead for CountingReader {
			fn poll_read(
				...
				let Some(available) = self.bytes.get(self.position..) else {
					return Poll::Ready(Err(std::io::Error::new(
						std::io::ErrorKind::InvalidData,
						"reader position exceeded the source buffer",
					)));
				};
				let read = available.len().min(buffer.remaining());
				let Some(chunk) = available.get(..read) else {
					return Poll::Ready(Err(std::io::Error::new(
						std::io::ErrorKind::InvalidData,
						"reader buffer ended before the requested chunk",
					)));
				};
