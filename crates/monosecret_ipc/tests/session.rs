...
async fn initializes_calls_and_shuts_down() {
	...
	let result: Value = client
		...
		.await
		.unwrap();
	assert_eq!(at(&result, "/value").as_i64(), Some(42));
	client
...
async fn discovery_is_side_effect_free_before_and_available_after_initialization() {
	...
	assert_eq!(
		at(&expired_reply, "/error/data/kind").as_str(),
	...
	assert_eq!(at(&discovered, "/result/openrpc").as_str(), Some("1.3.2"));
	assert_eq!(
		at(&discovered, "/result/x-monosecret/protocol").as_str(),
		Some("monosecret.resolver")
	);
	assert_eq!(
		at(&discovered, "/result/x-monosecret/server/name").as_str(),
		Some("discovery-test")
	);
	assert_eq!(
	...
	assert!(
		at(&discovered, "/result/methods")
			...
			.unwrap()
			.iter()
			.any(|method| method.get("name").and_then(Value::as_str) == Some("rpc.discover"))
	);
	assert!(at(&discovered, "/result/components/schemas").is_object());
	...
	assert_eq!(
		at(&discovered_again, "/result/x-monosecret/protocol").as_str(),
		Some("monosecret.resolver")
	);
	assert_eq!(witness.initialized.load(Ordering::SeqCst), 1);
...
async fn second_initialize_while_the_first_is_active_cancels_startup_and_closes() {
	...
	.unwrap();
	assert_eq!(at(&response, "/id").as_u64(), Some(2));
	assert_eq!(
		at(&response, "/error/data/kind").as_str(),
		Some("invalid_request")
	);
...
async fn invalid_initialization_is_distinct_from_unsupported_version() {
	for (mut request, expected) in [
		(
			{
				...
				*at_mut(&mut request, "/params/limits/max_in_flight") = json!(0);
				request
			},
			"invalid_params",
		),
		(
			{
				let mut request = initialize_request(1);
				*at_mut(&mut request, "/params/protocol") = json!("unknown.protocol");
				request
			},
			"unsupported_version",
		),
		(
			{
				let mut request = initialize_request(1);
				*at_mut(&mut request, "/params/versions") = json!([999]);
				request
			},
			"unsupported_version",
		),
		...
		let (mut io, server) = raw_server();
		*at_mut(&mut request, "/_meta/deadline_unix_ms") = json!(deadline(Duration::from_secs(2)));
		write_frame(&mut io, &serde_json::to_vec(&request).unwrap(), 1_048_576)
			.await
			.unwrap();
		let response: Value =
			serde_json::from_slice(&read_frame(&mut io, 1_048_576).await.unwrap().unwrap())
				.unwrap();
		assert_eq!(at(&response, "/error/data/kind").as_str(), Some(expected));
		assert!(read_frame(&mut io, 1_048_576).await.unwrap().is_none());
		server.await.unwrap().unwrap();
	}
}
...
mod callbacks {
	...
	fn callback_with_parent(
		...
		if let Some(parent_request_id) = parent_request_id {
			request
				.as_object_mut()
				.and_then(|object| object.get_mut("_meta"))
	...
	async fn an_advertised_callback_is_answered_mid_request() {
		...
		assert_eq!(at(&result, "/supported"), &json!(true));
		assert_eq!(at(&result, "/answer"), &json!({"echoed": "TOKEN"}));
	}
	...
	async fn an_unadvertised_callback_is_refused_without_reaching_the_client() {
		...
		assert_eq!(at(&result, "/supported"), &json!(false));
		assert_eq!(
			at(&result, "/answer"),
			&json!({"error": "capability_required"})
		);
	}
	...
	async fn a_completed_callback_id_cannot_be_reused() {
		...
		let peer = tokio::spawn(async move {
			...
			write_frame(
				&mut peer_io,
				&initialize_response(at(&initialize, "/id").as_u64().unwrap(), 1),
				1024 * 1024,
			)
			.await
			.unwrap();
	...
	async fn callback_parent_must_be_present_and_active() {
		for parent_request_id in [None, Some(999)] {
			...
			let peer = tokio::spawn(async move {
				...
				let call = read_frame(&mut peer_io, 32 * 1024).await.unwrap().unwrap();
				let call: Value = serde_json::from_slice(&call).unwrap();
				let deadline = at(&call, "/_meta/deadline_unix_ms").as_u64().unwrap();
				write_frame(
					&mut peer_io,
	...
	async fn cancelling_a_parent_cancels_its_running_callback() {
		...
		let peer = tokio::spawn(async move {
			...
			let call = read_frame(&mut peer_io, 32 * 1024).await.unwrap().unwrap();
			let call: Value = serde_json::from_slice(&call).unwrap();
			let parent_id = at(&call, "/id").as_u64().unwrap();
			let parent_deadline = at(&call, "/_meta/deadline_unix_ms").as_u64().unwrap();
			write_frame(&mut peer_io, &callback(8, parent_deadline), 32 * 1024)
				.await
				.unwrap();
			...
			write_frame(
				&mut peer_io,
				&serde_json::to_vec(
					&json!({"jsonrpc":"2.0","id":at(&shutdown, "/id").clone(),"result":{}}),
				)
				.unwrap(),
				32 * 1024,
			)
	...
	async fn terminal_parent_cancels_callback_and_discards_its_result() {
		...
		let peer = tokio::spawn(async move {
			...
			assert_eq!(at(&callback_response, "/id").as_u64(), Some(8));
			assert_eq!(
				at(&callback_response, "/error/data/kind").as_str(),
				Some("cancelled")
			);
