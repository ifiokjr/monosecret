"""Build the monosecret_py_native pyo3 extension before the tests import the
package.
...
"ensure the addon exists" harness. ``MONOSECRET_BIN`` (the CLI) is still
...
_PKG_DIR = _REPO_ROOT / "python" / "monosecret_py"


def _bin_name() -> str:
    return "monosecret.exe" if sys.platform == "win32" else "monosecret"


def _ensure_extension_and_bin() -> None:
    subprocess.run(["cargo", "build", "-p", "monosecret"], cwd=_REPO_ROOT, check=True)
    meta = subprocess.run(
    ...
    os.environ.setdefault("MONOSECRET_BIN", str(debug / _bin_name()))
