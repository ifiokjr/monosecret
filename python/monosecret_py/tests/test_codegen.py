"""End-to-end test of the typed-codegen pipeline:
    ...
    monosecret schema  ->  quicktype  ->  Type.from_dict(resolved.fields())
...
def _generate_types(tmp_path: pathlib.Path, name: str):
    manifest = tmp_path / "monosecret.toml"
    manifest.write_text(MANIFEST)

    schema = tmp_path / "schema.json"
    subprocess.run(
        [os.environ["MONOSECRET_BIN"], "-f", str(manifest), "schema", "-o", str(schema)],
        check=True,
    )
