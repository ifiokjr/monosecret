...
# Build the CLI (for `monosecret schema`) and compile the native extension (the
...
def build_artifacts
  ...
  File.join(meta["target_directory"], "debug", "monosecret")
end
...
class CodegenTest < Minitest::Test
  def test_quicktype_ruby_consumes_fields
    ...
    Dir.mktmpdir do |dir|
      manifest = File.join(dir, "monosecret.toml")
