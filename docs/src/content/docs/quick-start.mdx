---
title: Quick Start
description: Get up and running with Monosecret in minutes
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

## Installation

Choose your preferred installation method:

<Tabs>
<TabItem label="npm">

```bash
$ npm install --global @monosecret/cli
```

</TabItem>
<TabItem label="Devenv.sh">

Add to your `devenv.nix`:

```nix
{ config, ... }:
{
  # Secrets are automatically populated from monosecret.toml
  env.DATABASE_URL = config.monosecret.secrets.DATABASE_URL;
  env.REDIS_URL = config.monosecret.secrets.REDIS_URL;
}
```

</TabItem>
<TabItem label="Nix">

```bash
$ nix-env -iA monosecret -f https://github.com/NixOS/nixpkgs/tarball/nixpkgs-unstable
```

</TabItem>
</Tabs>

## Getting started

Start with one project and the system keyring. You will declare the secrets the
application expects, store one value, and run the application with that value
in its environment.

### 1. Initialize `monosecret.toml`

From your project directory, create a manifest:

```bash
$ monosecret init
```

<div class="muted-code-output">

```text
✓ Created monosecret.toml with 0 secrets

Next steps:
  1. monosecret config global init    # Set up user defaults (0.2+)
  2. monosecret check                 # Verify all secrets are set
  3. monosecret run -- your-command   # Run with secrets
```

</div>

This example assumes the project does not have a `.env` file. If one exists,
`init` discovers its secret names automatically and reports the number of
declarations it created. Review those declarations instead of replacing them
in the next step. See [Migration](/migration/) for details.

### 2. Declare your secrets

Edit `monosecret.toml` so it describes what your application expects:

```toml title="monosecret.toml"
[project]
name = "my-app"
revision = "1.0"

[profiles.default]
DATABASE_URL = { description = "PostgreSQL connection string", required = true }
SENTRY_DSN = { description = "Error reporting endpoint", required = false }
```

`DATABASE_URL` must be available before the application can run. `SENTRY_DSN`
is optional, so leaving it unset does not block resolution. The manifest
contains declarations, not secret values, and is safe to commit.

### 3. Store and use a secret

Store the required value in your system keyring:

```bash
$ monosecret set DATABASE_URL --provider keyring
```

<div class="muted-code-output">

```text
Enter value for DATABASE_URL (profile: default): ********
✓ Secret 'DATABASE_URL' saved to keyring (profile: default)
```

</div>

Start your application with the resolved values in its environment:

```bash
$ monosecret run --provider keyring -- npm start
```

### 4. Configure your personal defaults

The commands above use `--provider keyring` explicitly. Monosecret 0.2+ can
save your preferred backend and default profile as preferences for your user:

```bash
$ monosecret config global init  # 0.2+
? Select your preferred provider backend:
> keyring: Uses system keychain (Recommended)
  kdbx: KeePass KDBX databases (0.2+)
  onepassword: 1Password password manager
  keeper: Keeper Secrets Manager (0.2+) via official Rust SDK
  dotenv: Traditional .env files
  file: Plaintext files, one per secret (0.2+)
  env: Read-only environment variables
  null: Use defaults, generation, or run prompts without storage (0.2+)
  systemd-credential: Read-only systemd service credentials (0.2+)
  pass: Unix password manager with GPG encryption
  gopass: Gopass CLI password manager with GPG encryption (0.2+)
  protonpass: Proton Pass via official pass-cli
  passbolt: Passbolt self-hosted password manager (0.2+) via go-passbolt-cli
  lastpass: LastPass password manager
  dashlane: Dashlane password manager, read-only (0.2+)
  gcsm: Google Cloud Secret Manager
  awssm: AWS Secrets Manager
  awsps: AWS Systems Manager Parameter Store (0.2+)
  scaleway: Scaleway Secret Manager (0.2+)
  vault: HashiCorp Vault secret management
  openbao: OpenBao secret management (0.2+)
  bw: Bitwarden Password Manager (0.2+)
  bws: Bitwarden Secrets Manager
  akv: Azure Key Vault
  infisical: Infisical secret management (0.2+)
  age: age-encrypted file (0.2+)
  sops: SOPS encrypted files (0.2+)
? Select your default profile:
  development
> default
  none
✓ Configuration saved to /home/user/.config/monosecret/config.toml
```

These preferences are stored in `~/.config/monosecret/config.toml`. They are
not written to the project, committed to version control, or shared with other
users. They become your personal defaults across projects and can still be
overridden by project configuration or command-line options.

You can now omit the provider from everyday commands:

```bash
$ monosecret set DATABASE_URL
$ monosecret check
$ monosecret run -- npm start
```

## Next Steps

- Continue with the commands in [Basic Usage](/basic-usage/)
- Bring existing values into Monosecret with the [Migration guide](/migration/)
- Learn about [Profiles](/concepts/profiles/) to manage environment-specific configurations
- Explore different [Providers](/concepts/providers/) for secret storage
- Choose an [SDK](/sdk/overview/) to resolve secrets from your application
