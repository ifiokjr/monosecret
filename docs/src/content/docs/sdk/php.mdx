...
> **Planned for a future release:** The source is integrated for local testing,
...
`monosecret_ffi` was named `monosecret-ffi` through Monosecret 0.3. The PHP
loader prefers the new artifact name and still accepts pre-0.3 shared
libraries.
:::
...
array (Monosecret 0.4.0+). The embedded extension or FFI fallback uses the
