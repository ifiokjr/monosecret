...
Node.js/TypeScript, Haskell, PHP, C#, and Swift (0.3+). PHP, C#, and
Swift registry/binary publication is deferred; their package commands document
planned usage. They all resolve secrets from the same
declarative `monosecret.toml`, and they all behave identically, because they
share one resolver.

> **C# compatibility:** Package publication is deferred to a future release.
...
- **C# (0.3+)** loads the same C ABI with P/Invoke from a runtime-specific
  native asset in the NuGet package.
- **Swift (0.3+)** imports the same C ABI as a Clang module from a macOS
...
[C# (0.3+)](/sdk/csharp), and [Swift (0.3+)](/sdk/swift).

Every builder also takes a [scope (0.3+)](/concepts/scopes/),
...
- **C# (0.3+)** ships the `cdylib` as runtime-specific native assets in one
...
- **Swift (0.3+)** ships Intel and Apple-silicon macOS cdylibs in one
