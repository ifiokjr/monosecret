...
schema v2, which requires Monosecret 0.4.0+.
...
Inline specification v2 (Monosecret 0.4.0+) uses `project`, `profiles`, and each
...
supported. A 0.4.0+ library still accepts the v1 documents sent by earlier SDKs.
Unknown declaration fields are rejected. `project.extends` resolves parent
manifests relative to `baseDir`.
The SDK requires the native `monosecret_call` capability for inline specs; an
