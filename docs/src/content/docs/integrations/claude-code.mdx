...
The Claude Code credential integration is available in Monosecret 0.4.0+. It
...
- Monosecret 0.4.0 or newer, with `monosecret` on `PATH`
...
and the helper also ignore an exported `MONOSECRET_PROVIDER`, so they always
use the same store: the provider pinned by `configure`, or the Monosecret user
