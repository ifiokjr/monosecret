...
| [file](/providers/file/) (0.2+) | One plaintext byte file per secret (arbitrary bytes in 0.4.0+) | ✓ | ✓ | ✗ | — |
| [env](/providers/env/) | Current process environment | ✓ | ✗ | ✗ | — |
| [null](/providers/null/) (0.3+) | No storage; uses a manifest default, ephemeral generation, or an ephemeral run prompt | ✗ | ✗ | N/A | — |
| [systemd-credential](/providers/systemd-credential/) (0.3+) | Credentials passed to the current systemd service (arbitrary bytes in 0.4.0+) | ✓ | ✗ | Depends on the unit's credential source | [Via systemd-creds](https://www.freedesktop.org/software/systemd/man/latest/systemd-creds.html) |
| [fly](/providers/fly/) (0.3+) | Fly.io application secrets through `flyctl` | ✗ | ✓ | ✓ | — |
| [cloudflare](/providers/cloudflare/) (0.3+) | Cloudflare account-level Secrets Store through its REST API | ✗ | ✓ | ✓ | — |
...
| [doppler](/providers/doppler/) (0.4.0+) | Doppler over its REST API (included by default; `doppler` feature for custom builds) | ✓ | ✓ | ✓ | — |
| [gcsm](/providers/gcsm/) | Google Cloud Secret Manager (requires the `gcsm` build feature) | ✓ | ✓ | ✓ | — |
| [awssm](/providers/awssm/) | AWS Secrets Manager, including `SecretBinary` (0.4.0+; requires the `awssm` build feature) | ✓ | ✓ | ✓ | — |
...
| [setec](/providers/setec/) (0.4.0+) | Tailscale Setec (included by default; `setec` feature for custom builds) | ✓ | ✓ | ✓ | Depends on server key configuration |
...
| [aac](/providers/aac/) (0.3+) | Azure App Configuration, including Key Vault-reference resolution (included by default; `aac` feature for custom builds) | ✓ | ✓ | ✓ | — |
| [infisical](/providers/infisical/) (0.3+) | Infisical (requires the `infisical` build feature) | ✓ | ✓ | ✓ | — |
| [age](/providers/age/) (0.3+) | An age-encrypted file (requires the `age` build feature) | ✓ | ✓ | ✓ | — |
| [sops](/providers/sops/) (0.3+) | SOPS-encrypted files (requires the `sops` build feature and SOPS CLI) | ✓ | ✓ | ✓ | Depends on the configured SOPS key service |
| [kubernetes](/providers/kubernetes/) (0.3+) | Kubernetes ConfigMaps and Secrets (requires the `kubernetes` build feature) | ✓ | ✓ | [Secrets can be encrypted at rest](https://kubernetes.io/docs/tasks/administer-cluster/encrypt-data/#ensure-all-secrets-are-encrypted) | — |
...
## Configure a project default provider chain (0.4.0+)
...
- **Configured credentials are authoritative.** When an alias declares a
  ...
  supplied.
- **Credential bytes are preserved (0.4.0+).** Resolution keeps each credential
...
- **External providers negotiate names (0.4.0+).** An out-of-tree endpoint asks
  for the URI-specific credentials it needs at runtime. Its alias does not need
  a `credentials` table: Monosecret can load requested values from a
...
- **A URI may not carry a credential (0.3+).** A provider URI with a password
