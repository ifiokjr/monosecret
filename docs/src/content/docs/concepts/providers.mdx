...
| [null](/providers/null/) (0.2+) | No storage; uses a manifest default, ephemeral generation, or an ephemeral run prompt | ✗ | ✗ | N/A | — |
| [systemd-credential](/providers/systemd-credential/) (0.2+) | Credentials passed to the current systemd service | ✓ | ✗ | Depends on the unit's credential source | [Via systemd-creds](https://www.freedesktop.org/software/systemd/man/latest/systemd-creds.html) |
...
| [infisical](/providers/infisical/) (0.2+) | Infisical (requires the `infisical` build feature) | ✓ | ✓ | ✓ | — |
| [age](/providers/age/) (0.2+) | An age-encrypted file (requires the `age` build feature) | ✓ | ✓ | ✓ | — |
| [sops](/providers/sops/) (0.2+) | SOPS-encrypted files (requires the `sops` build feature and SOPS CLI) | ✓ | ✓ | ✓ | Depends on the configured SOPS key service |
...
- **A URI may not carry a credential (0.2+).** A provider URI with a password
