...
the profile the run resolves under (0.3+) — the same environment a convention secret reads in that
run.
...
- A ref reads the environment the profile names unless `?env=` pins one. Infisical answers a
  missing secret, folder, environment and project with the same 404. Starting in Monosecret 0.3,
