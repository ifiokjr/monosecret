...
both rejected through Monosecret 0.3). `field` remains unsupported. Monosecret
0.3+ accepts `version` as a 32-character ASCII alphanumeric Azure Key Vault
...
DATABASE_URL = {
  description = "DB",
  ref = { item = "database-url", version = "0123456789abcdef0123456789abcdef" }, # version: 0.3+
