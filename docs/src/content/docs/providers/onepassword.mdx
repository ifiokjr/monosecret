...
1Password bills service accounts per request, and every secret reference
...
- One auth probe per run (`op vault list`), deduplicated across every
  provider instance.
- All referenced items in a vault are served by one batched
  `op item get` — one billed read per *item*, however many secrets read
  fields of it. A whole profile pinned to a single shared item (the
  `op+token://Vault/Item` layout) costs that one read, not one per secret.
- Convention secrets are served by one `op item list` plus one batched
  `op item get` per vault.
- `op inject` (and per-secret reads) remain only as a correctness
  fallback for references the item reads cannot serve, such as ambiguous
  item titles.
