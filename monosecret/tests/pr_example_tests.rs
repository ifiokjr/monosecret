...
use monosecret::ProviderConfig;
use monosecret::ProviderRef;
use monosecret::ProviderRefDetail;
use monosecret::SecretRequest;
use tempfile::TempDir;

fn write_and_parse(toml_content: &str) -> Config {
	let dir = TempDir::new().unwrap();
	let path = dir.path().join("monosecret.toml");
...
fn example_5_cross_project_shared_config() {
	...
	fs::write(
		shared_dir.join("monosecret.toml"),
		r#"
[project]
name = "team-shared"
revision = "1.0"
...
"#,
	...
	fs::write(
		myapp_dir.join("monosecret.toml"),
		r#"
[project]
name = "myapp"
revision = "1.0"
...
"#,
	...
	let config = Config::try_from(myapp_dir.join("monosecret.toml").as_path()).unwrap();
	assert_eq!(config.project.name, "myapp");
	assert_eq!(
		config.project.extends,
		Some(vec!["../team-shared".to_string()])
	);
...
"#,
	...
	assert!(
		matches!(providers.get("dotenv"), Some(ProviderConfig::Alias(s)) if s == "dotenv://.env.local")
	);
}
